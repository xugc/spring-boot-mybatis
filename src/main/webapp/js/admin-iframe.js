	...
	if($("#centerPanel").tabs('exists',t)){
		...
		$("#centerPanel").tabs('add',
			...
			content : '<iframe style="width: 100%;height: 100%;" src="'
				+ url + '" frameBorder=0 scrolling="auto"></iframe>',
				closable : true
				});
	}
}
$(function() {
	$(".top_nav").find("li").each(
			function() {
				var liEl = $(this);
				$(liEl).find("a").click(
						function() {
							...
							if (mindex == 0) {
								...
								$("#main").hide();
								var lmenu = $(".left").find(
										"ul[id='left_menu_" + mindex + "']");
								$(lmenu).show();
								var targetUrl = $(lmenu).find(
										"a[class='selected']").attr("turl");
								var targetTitle = $(lmenu).find(
